...
public class IntersectionPoint implements ReferencePoint {
	...
	public IntersectionPoint(final int index,
			final Identifier<? extends Form> formAId,
			final Identifier<? extends Form> formBId) {
		_index = index;
		_formAId = formAId;
		_formBId = formBId;
	}
	...
	public boolean isValidFor(final Runtime runtime) {
		...
		return !Double.isNaN(Intersector.intersectXForRuntime(runtime,
				outlineA, outlineB, _index));
	}
	...
	public double getXValueForRuntime(final Runtime runtime) {
		...
		return Intersector.intersectXForRuntime(runtime, outlineA, outlineB,
				_index);
	...
	public double getYValueForRuntime(final Runtime runtime) {
		...
		return Intersector.intersectYForRuntime(runtime, outlineA, outlineB,
				_index);
	}
	...
	public String getDescription(final Analyzer analyzer) {
		...
		return "Intersection#" + (_index + 1) + " of "
		+ (formA != null ? formA.getName().getValue() : "???")
				+ " and "
