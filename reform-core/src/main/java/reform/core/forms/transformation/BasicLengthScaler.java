...
public class BasicLengthScaler implements Scaler {
	...
	public void scale(final Runtime runtime, final double factor,
		...
		if (directionX == 0 && directionY == 0) {
			...
			final double projectedX = Vector.projectionX(deltaX, deltaY,
					directionX, directionY);
			final double projectedY = Vector.projectionY(deltaX, deltaY,
					directionX, directionY);

			final double scaledX = deltaX + projectedX * (factor - 1);
			final double scaledY = deltaY + projectedY * (factor - 1);

			final double scaledLength = Math.sqrt(scaledX * scaledX + scaledY
					* scaledY)
					/ l;
