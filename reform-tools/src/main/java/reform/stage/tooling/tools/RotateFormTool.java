...
public class RotateFormTool implements Tool {
	...
	public RotateFormTool(final SelectionTool selectionTool,
	...
	public void press() {
		if (_state == State.Snapped) {
			...
			_currentInstruction = new RotateInstruction(
					_currentHandle.getFormId(), _currentAngle, pivotPoint);
	...
	public void input(final Input input) {
		...
		case Pressed: {
			_currentAngle
					.setAngle(calcAngle(input.getShiftModifier().isActive()));
	...
	private double calcAngle(final boolean stepped) {
		...
		final double angle = -Vector.angle(_cursor.getX() - _currentOffset.x,
				_cursor.getY() - _currentOffset.y, pivotPos.x, pivotPos.y)
				+ Vector.angle(_startPosition.x, _startPosition.y, pivotPos.x,
						pivotPos.y);
