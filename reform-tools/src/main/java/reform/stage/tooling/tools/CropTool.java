...
public class CropTool implements Tool {
	...
	public CropTool(final ToolState toolState, final Cursor cursor,
			final EventedPicture ePicture) {
	...
	public void setUp() {
		...
		_toolState.setCropPointsVisible(true);
	...
	public void tearDown() {
		_toolState.clearSnapPoints();
		_toolState.setCropPointsVisible(false);
	...
	public void input(final Input input) {
		...
		case Pressed: {
			...
			_eventedPicture
					.setSize(calcSize(input.getShiftModifier().isActive()));
		}
		}
	...
	private Vec2i calcSize(final boolean keepRatio) {
		...
		final int newHeight = Math.max(_eventedPicture.getSize().y + deltaY,
				10);
		...
		if (keepRatio && _currentPoint.isCorner()) {
			...
			return new Vec2i((int) Math.round(min),
					(int) Math.round(min * ratio));
		} else {
