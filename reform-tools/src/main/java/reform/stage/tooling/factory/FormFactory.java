...
public class FormFactory<T>
{
	...
	public FormFactory(final String name, final IdentifierEmitter idEmitter, final Builder<T> builder)
	{
		_namePrefix = name;
	...
	public T build()
	{
		return _builder.build(_idEmitter.emit(), new Name(_namePrefix + " " + _counter++));
	}
}
