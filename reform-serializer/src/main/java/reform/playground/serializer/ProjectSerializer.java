...
import reform.core.forms.relations.ConstantPoint;
...
import reform.core.procedure.instructions.single.CreateGroupInstruction;
import reform.core.procedure.instructions.single.ErrorInstruction;
...
public class ProjectSerializer {
	...
	private Project readProject(final JSONObject jsonObject) {
		...
		for (final Picture p : readPictures(
				jsonObject.getJSONArray("pictures"))) {
	...
	private Picture readPicture(final JSONObject object) {
		...
		final Procedure procedure = readProcedure(
				object.getJSONObject("procedure"));
	...
	private void readInstructionsInto(final Procedure procedure,
			final InstructionGroup root, final JSONArray jsonArray) {
	...
	private Instruction readInstruction(final Procedure procedure,
	...
	private CreateFormInstruction readCreateFormInstruction(
	...
	private InitialDestination readDestination(final JSONObject jsonObject) {
		...
		if (type.equals(RelativeDynamicSizeDestination.class.getSimpleName())) {
			...
			if (type.equals(RelativeFixSizeDestination.class.getSimpleName())) {
	...
	private RelativeFixSizeDestination readRelativeFixSizeDestination(
			final JSONObject jsonObject) {
	...
	private InitialDestination readRelativeDynamicSizeDestination(
			final JSONObject jsonObject) {
	...
	private Form readForm(final JSONObject jsonObject) {
		...
		if (type.equals(LineForm.class.getSimpleName())) {
			form = LineForm.construct(readId(jsonObject, "id"),
					readName(jsonObject));
			...
			form = CircleForm.construct(readId(jsonObject, "id"),
					readName(jsonObject));
		} else if (type.equals(ArcForm.class.getSimpleName())) {
			form = ArcForm.construct(readId(jsonObject, "id"),
					readName(jsonObject));
		} else if (type.equals(PieForm.class.getSimpleName())) {
			form = PieForm.construct(readId(jsonObject, "id"),
					readName(jsonObject));
		} else if (type.equals(PictureForm.class.getSimpleName())) {
			form = PictureForm.construct(readId(jsonObject, "id"),
					readName(jsonObject));
		} else {
			throw new SerializationError(type);
		}
	...
	private TranslationDistance readRelativeDistance(
			final JSONObject jsonObject) {
	...
	private TranslationDistance readConstantDistance(
			final JSONObject jsonObject) {
		return new ConstantDistance(
				readVector(jsonObject.getJSONObject("delta")));
	}
	...
	private ReferencePoint readIntersectionPoint(final JSONObject jsonObject) {
		return new IntersectionPoint(jsonObject.getInt("index"),
				readId(jsonObject, "formAId"), readId(jsonObject, "formBId"));
	}
	...
	private RotationAngle readConstantRotationAngle(
			final JSONObject jsonObject) {
	...
	private ForLoopInstruction readForLoopInstruction(final Procedure procedure,
			final JSONObject object) {
		final ForLoopInstruction loop = new ForLoopInstruction(
				object.getInt("times"));
	...
	private IfConditionInstruction readIfConditionInstruction(
	...
	private void writeProcedure(final JSONWriter writer,
	...
	private void writeGroupChildren(final JSONWriter writer,
			final InstructionGroup root) {
		writer.array();
	...
	private void writeInstruction(final JSONWriter writer,
		...
		if (cls == CreateFormInstruction.class) {
			writeCreateFormInstruction(writer,
					(CreateFormInstruction) instruction);
			...
			writeTranslateInstruction(writer,
			...
			writeIfConditionInstruction(writer,
					(IfConditionInstruction) instruction);
	...
	private void writeForLoopInstruction(final JSONWriter writer,
			final ForLoopInstruction instruction) {
		writer.key("times");
		writer.value(instruction.getTimes());
	}

	private void writeTranslateInstruction(final JSONWriter writer,
			final TranslateInstruction instruction) {
		writer.key("form");
	...
	private void writeScaleInstruction(final JSONWriter writer,
			final ScaleInstruction instruction) {
		writer.key("form");
		writeId(writer, instruction.getFormId());
		writer.key("factor");
	...
	private void writeRotateInstruction(final JSONWriter writer,
			final RotateInstruction instruction) {
		writer.key("form");
		writeId(writer, instruction.getFormId());
		writer.key("angle");
	...
	private void writeMorphInstruction(final JSONWriter writer,
			final MorphInstruction instruction) {
		writer.key("form");
		writeId(writer, instruction.getFormId());
		writer.key("anchor");
	...
	private void writeCreateFormInstruction(final JSONWriter writer,
			final CreateFormInstruction instruction) {
		writer.key("form");
	...
	private void writeRotationAngle(final JSONWriter writer,
	...
	private void writeConstantAngle(final JSONWriter writer,
			final ConstantAngle angle) {
		writer.key("value");
		writer.value(angle.getValue());
	}

	private void writeScaleFactor(final JSONWriter writer,
			final ScaleFactor factor) {
	...
	private void writeConstantScaleFactor(final JSONWriter writer,
			final ConstantScaleFactor factor) {
		writer.key("value");
		writer.value(factor.getValue());
	}

	private void writeTranslationDistance(final JSONWriter writer,
	...
	private void writeRelativeDistance(final JSONWriter writer,
	...
	private void writeConstantDistance(final JSONWriter writer,
			final ConstantDistance distance) {
		writer.key("delta");
		writeVector(writer, distance.getDelta());
	}

	private void writeDestination(final JSONWriter writer,
			final InitialDestination destination) {
		...
		if (cls == RelativeDynamicSizeDestination.class) {
			writeRelativeDynamicSizeDestination(writer,
					(RelativeDynamicSizeDestination) destination);
	...
	private void writeRelativeFixSizeDestination(final JSONWriter writer,
			final RelativeFixSizeDestination destination) {
	...
	private void writeRelativeDynamicSizeDestination(final JSONWriter writer,
			final RelativeDynamicSizeDestination destination) {
	...
	private void writeAlignment(final JSONWriter writer,
	...
	private void writeDirection(final JSONWriter writer,
		...
		if (direction instanceof CartesianDirection) {
			...
			writer.key("type");
			writer.value(cls.getSimpleName());
			writeProportionalDirection(writer,
					(ProportionalDirection) direction);
	...
	private void writeCartesianDirection(final JSONWriter writer,
			final CartesianDirection direction) {
		writer.key("value");
		writer.value(direction.name());
	}

	private void writeProportionalDirection(final JSONWriter writer,
			final ProportionalDirection direction) {
	...
	private void writeConstantPoint(final JSONWriter writer,
			final ConstantPoint point) {
		writer.key("value");
		writeVector(writer, point.getValue());
	}

	private void writeIntersectionPoint(final JSONWriter writer,
			final IntersectionPoint point) {
		writer.key("index");
	...
	private void writeForeignFormsPoint(final JSONWriter writer,
			final ForeignFormsPoint p) {
		writer.key("formId");
