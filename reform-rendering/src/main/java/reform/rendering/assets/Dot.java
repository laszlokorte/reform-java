...
public class Dot implements Asset {
	...
	public void drawAt(final Graphics g, final double x, final double y) {
		...
		g.drawImage(_bitmap, (int) (Math.round(x) - w / 2),
				(int) (Math.round(y) - h / 2), w, h, null);
	}

	private void _prerender(final float strokeWidth, final Color fillColor,
		...
		g.setPaint(new GradientPaint(0, 0, fillColor.brighter(), 0, _bitmap
				.getHeight(), fillColor));
		g.fillOval(strokeWidthInt, strokeWidthInt, _bitmap.getWidth() - 2
				* strokeWidthInt, _bitmap.getHeight() - 2 * strokeWidthInt);
		g.setColor(strokeColor);
		g.setStroke(stroke);
		g.drawOval(strokeWidthInt, strokeWidthInt, _bitmap.getWidth() - 2
				* strokeWidthInt, _bitmap.getHeight() - 2 * strokeWidthInt);
		g.setColor(strokeColor);
	}
