...
public class ShapeTextIcon implements Icon {
	...
	public ShapeTextIcon() {
		...
		final GlyphVector v = f.createGlyphVector(
				metrics.getFontRenderContext(), "T");
		final Shape s = v.getOutline();
		final Rectangle bounds = s.getBounds();
		_size = Math.max(bounds.width, bounds.height) + 10;
		_shape = AffineTransform.getTranslateInstance(
	...
	public void draw(final Graphics2D g, final int x, final int y,
			final int width) {
		final AffineTransform t = AffineTransform.getScaleInstance(width
				/ _size, width / _size);
		g.translate(x, y);
