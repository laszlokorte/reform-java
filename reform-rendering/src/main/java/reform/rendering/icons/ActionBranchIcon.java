...
public class ActionBranchIcon implements Icon {
	...
	public ActionBranchIcon() {
		...
		final Area tip = new Area(
				t.createTransformedShape(new Rectangle2D.Double(-175, -175,
						350, 350)));
		...
		_shape.add(branch.createTransformedArea(AffineTransform
				.getTranslateInstance(-350, -400)));

		final AffineTransform t3 = AffineTransform.getTranslateInstance(-320,
				-60);
		t3.rotate(Math.PI / 2);
